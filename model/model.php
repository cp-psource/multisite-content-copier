...
class Multisite_Content_Copier_Model {
	...
	public function add_synced_content( $src_post_id, $src_blog_id, $dest_posts, $settings = array() ) {
		...
		if ( ! is_array( $dest_posts ) || empty( $dest_posts ) )
			return new WP_Error( 'dest_posts',  __( "Zielposts müssen ein Array sein", MULTISTE_CC_LANG_DOMAIN ) );
		...
		if ( count( $dest_blogs_ids ) != count( $dest_posts_ids ) )
			return new WP_Error( 'dest_posts',  __( "Zielposts müssen ein kohärentes Array sein", MULTISTE_CC_LANG_DOMAIN ) );
		...
		if ( empty( $insert ) )
			return new WP_Error( 'dest_posts',  __( "Nichts einzufügen", MULTISTE_CC_LANG_DOMAIN ) );
