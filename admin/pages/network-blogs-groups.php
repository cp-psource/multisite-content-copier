	...
	public function __construct( $menu_slug, $capability, $args ) {
 	...
 	public function render_content() {
 		...
 		if ( 'nbt' == $this->get_current_tab() ) {
			...
			<form action="<?php echo esc_url( add_query_arg( 'tab', 'nbt', $this->get_permalink() ) ); ?>" method="post" id="template-search">
				<?php $wp_list_table->search_box( __( 'Suche nach Vorlagennamen', MULTISTE_CC_LANG_DOMAIN ), 'template' ); ?>
 	...
 	public function add_help_tabs() {
        ...
        $screen->add_help_tab( array(
	        ...
	        'title'	=> __( 'Webseiten-Gruppen', MULTISTE_CC_LANG_DOMAIN ),
	        'content'	=> '<h4>' . __( 'Was ist eine Webseiten-Gruppe?', MULTISTE_CC_LANG_DOMAIN ) . '</h4>',
	    ) );
 	}
 	...
 	public function render_group_edit_screen() {
 		...
 		if ( empty( $group ) ) {
 			?>
				<p><?php _e( 'Die Gruppe existiert nicht', MULTISTE_CC_LANG_DOMAIN ); ?></p>
 			<?php
 		}
 		else {
 			if ( isset( $_GET['updated'] ) ) {
	 			?>
					<div class="updated"><p><?php _e( 'Die Gruppe wurde aktualisiert.', MULTISTE_CC_LANG_DOMAIN ); ?> <a href="<?php echo esc_url( $this->get_permalink() ); ?>"><?php _e( 'Zurück zur Gruppenliste', MULTISTE_CC_LANG_DOMAIN ); ?></a></p></div>
	 			<?php
	 		}
	 		elseif ( mcc_is_error() ) {
	 			mcc_show_errors();
 			...
 			?>
 				<form action="" method="post">
					<table class="form-table">
						<?php $this->render_row( __( 'Gruppenname', MULTISTE_CC_LANG_DOMAIN ), array( &$this, 'render_group_name_field' ) ); ?>
					</table>
					<p class="submit">
						<input type="hidden" name="group_id" value="<?php echo $group_id; ?>">
						<?php wp_nonce_field( 'edit-mcc-group', '_wpnonce' ); ?>
						<?php submit_button( __( 'Änderungen speichern', MULTISTE_CC_LANG_DOMAIN ), 'primary', 'submit_edit_group', false ); ?>
						<a href="<?php echo esc_url( $this->get_permalink() ); ?>" class="button-secondary"><?php _e( 'Abbrechen', MULTISTE_CC_LANG_DOMAIN ); ?></a>
					</p>
				</form>
	 		<?php
 		}
 	...
 	public function render_groups_screen() {
 		...
 		if ( isset( $_GET['added'] ) ) {
 			?>
				<div class="updated"><p><?php _e( 'Die Gruppe wurde hinzugefügt', MULTISTE_CC_LANG_DOMAIN ); ?></p></div>
 			<?php
 		}
 		elseif ( mcc_is_error() ) {
 			mcc_show_errors();
 		...
 		?>
			...
			<div id="col-container">
				...
				<div id="col-left">
					<div class="col-wrap">
						<div class="form-wrap">
							<h3><?php _e( 'Neue Gruppe hinzufügen', MULTISTE_CC_LANG_DOMAIN ); ?></h3>
							<form id="mcc-groups-table-form" action="" method="post">
								<?php wp_nonce_field( 'add-mcc-group' ); ?>
								<div class="form-field">
									<label for="group_name"><?php _e( 'Gruppenname', MULTISTE_CC_LANG_DOMAIN ); ?></label>
									<input name="group_name" id="group_name" type="text" value="<?php echo $group_name; ?>" size="40" aria-required="true"><br/>
								</div>
								<p class="submit"><input type="submit" name="submit_new_group" id="submit_new_group" class="button button-primary" value="<?php _e( 'Neue Gruppe hinzufügen', MULTISTE_CC_LANG_DOMAIN ); ?>"></p>
							</form>
						</div>
					</div>
				</div>
 	...
 	public function validate_form() {
 		if ( isset( $_POST['submit_new_group'] ) ) {
 			...
 			if ( empty( $group_name ) )
 				mcc_add_error( 'empty-group-name', __( 'Gruppenname darf nicht leer sein', MULTISTE_CC_LANG_DOMAIN ) );
